...
# first end communicates with back end using port 5001
...
def ask_with_context():
    ...
    qa_task_prompt = load_prompt("context_qa_task_prompt.txt")
    ...
    implementation_system_prompt = f"{implementation_system_prompt}\n\n {question}"
    qa_task_prompt = f"{qa_task_prompt}\n\n {question}"
    ...
    system_prompt = client.chat.completions.create(
        model=OLLAMA_MODEL,
        messages=[
            # for internal system not for the user :P
        ],
        max_tokens=300,
        temperature=0
    )
    ...
    implementation_system_prompt = f"<p>{final_prompt.choices[0].message.content} {question}</p>"
    qa_task_output = f"<p>{final_prompt.choices[0].message.content} {question}</p>"
    ...
    try:
        ...
        return "<p>Context-aware request failed.</p>", 503
